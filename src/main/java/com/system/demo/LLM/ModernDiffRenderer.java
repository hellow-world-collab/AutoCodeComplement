...
public class ModernDiffRenderer {
    ...
    private void handleUndo() {
        ...
        int result = Messages.showYesNoDialog(
                project,
                String.format("确认撤销全部 %d 个未处理的 AI 建议，恢复原代码吗？", unprocessedCount),
                "全局 Undo",
                Messages.getQuestionIcon()
        );
        ...
        WriteCommandAction.runWriteCommandAction(project, () -> {
            try {
                ...
                for (int i = chunks.size() - 1; i >= 0; i--) {
                    ...
                    if (chunk.modifiedStartLine >= 0 && chunk.modifiedStartLine < doc.getLineCount()) {
                        int start = doc.getLineStartOffset(chunk.modifiedStartLine);
                        int endLine = chunk.modifiedStartLine + chunk.delta.getTarget().size() - 1;
                        ...
                        // 确保不越界
                        if (start < doc.getTextLength() && endOffset <= doc.getTextLength() && start < endOffset) {
                            doc.deleteString(start, endOffset);
                        }
                    }

                    // 情况2: 有原代码需要保留（去掉删除线）
                    if (chunk.originalStartLine >= 0 && chunk.originalStartLine < doc.getLineCount()) {
                        // 原代码已经在文档中，只是有删除线，我们不需要做任何操作
                        // 因为撤销操作就是保留原代码
                    }
                ...
                ApplicationManager.getApplication().invokeLater(() -> {
                    Messages.showInfoMessage(project, "已撤销全部 AI 建议，恢复原代码", "完成");
                    closeToolbar();
                });

            } catch (Exception e) {
                Messages.showErrorDialog(project, "批量撤销时出错: " + e.getMessage(), "错误");
            }
        });
    }
