...
### 2. 代码分析与重构
- **分析快捷键**：`Shift + Alt + 1` 发送选中代码给 AI 分析和优化
- **注释快捷键**：`Shift + Alt + 3` 为选中代码添加专业注释
- **应用快捷键**：`Shift + Alt + 2` 应用 AI 的修改建议
- **显示方式**：专业的 Diff 对比视图（模态对话框，防止误编辑）
...
- ✅ **智能缓存系统**：
  ...
  - 缓存过期时间 60 秒
  - 最大缓存容量 100 条记录
- ✅ **HTTP 代理支持**：预配置 127.0.0.1:7897
- ✅ **请求管理**：自动取消过期请求，避免资源浪费
- ✅ 可配置触发延迟，避免频繁调用
- ✅ 智能清理 LLM 返回内容（去除 markdown 标记等）
- ✅ 专业的 Diff 对比视图（使用 java-diff-utils）
- ✅ **模态对话框保护**：查看差异时锁定编辑器，防止误操作
- ✅ 后台线程调用 API，不阻塞 UI
- ✅ 持久化配置存储
- ✅ **多功能支持**：代码优化 + 代码注释
...
3. **代码分析与优化**
   - 选中需要优化的代码
   - 按 `Shift + Alt + 1` 发送给 AI
   - 在 Diff 窗口查看对比
   - 关闭窗口后按 `Shift + Alt + 2` 应用
...
- [ ] 支持更多 IDE 版本
- [x] 添加本地缓存，减少 API 调用（已完成）
- [ ] 支持多种补全策略（单行/多行/整个函数）
- [ ] 添加补全历史记录
- [ ] 支持快捷键自定义
- [ ] 添加统计和分析功能
- [ ] 可配置的代理设置
- [ ] 支持更多的代码操作（重构、解释、测试生成等）
